...
export default function UserOcrPage() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // ── Fetch FORM templates (not letter templates) ────────
    ...
    const handleFieldChange = (key, value) => {
        setMappedData(prev => ({ ...prev, [key]: value }));
    };

    const handleAddField = () => {
        const fieldName = prompt('Enter field name:');
        if (fieldName) {
            setMappedData(prev => ({ ...prev, [fieldName]: '' }));
        }
    };
        ...
        <div className="dashboard-container user-theme">
            ...
            <nav className="dashboard-nav">
                ...
                <div className="nav-info">
                    ...
                    <button className="signout-btn" onClick={() => navigate('/user/ocr-services')}>
                        ← Back
                    </button>
                </div>
            </nav>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {templates.map(t => (
                                    <div
                                        ...
                                        style={{
                                            padding: '12px 16px',
                                            border: `1.5px solid ${selectedTemplate?.templateId === t.templateId ? '#6d28d9' : '#e5e7eb'}`,
                                            borderRadius: 8,
                                            cursor: 'pointer',
